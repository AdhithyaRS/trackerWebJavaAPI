...
public class CustomerJwtFilter implements Filter {
    ...
    public CustomerJwtFilter(JwtUtils jwtUtils) {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (cookies != null) {
            ...
            for (Cookie cookie : cookies) {
                if ("jwt_signIn_token_customer".equals(cookie.getName())) {
                    String jwtToken = cookie.getValue();
                    try {
                        jwtUtils.validateToken(jwtToken);
                        tokenIsValid = jwtUtils.getUserTypeFromToken(jwtToken).equals( ""+UserType.CUSTOMER);
